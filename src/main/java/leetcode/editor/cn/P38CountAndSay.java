        ...
        public String countAndSay(int n){
            ...
            for (int i = 1; i < n; i++) {
                ...
                while (start < prev.length()){
                    while (end < prev.length() && prev.charAt(start) == prev.charAt(end)){
                        end++;
                    }
                    cur.append(end-start).append(prev.charAt(start));
                    start = end;
                }
            }
            return cur.toString();
                ...
                if(s.length() == 1){
                    ...
                    for (int i = 1; i < s.length(); i++) {
                        if(s.charAt(i-1) == s.charAt(i)){
                            ...
                            sb.append(t).append(s.charAt(i-1));
                            t = 1;
                        }
                        if(t == s.length() || i == s.length() - 1){
                            sb.append(t).append(s.charAt(i));
                        }
                    }
                }
                s = sb.toString();
